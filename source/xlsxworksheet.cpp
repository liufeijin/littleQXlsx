...
//	writer.writeEndElement();//dimension
    ...
    if (!d->VTopLeftCell.isEmpty())
       writer.writeAttribute(QStringLiteral("topLeftCell"), d->VTopLeftCell);
    if (!d->VZoomScaleNormal.isEmpty())
       writer.writeAttribute(QStringLiteral("zoomScaleNormal"), d->VZoomScaleNormal);
...
{   // Liu fei jin 2019/12/2
    ...
    while (!reader.atEnd() && !(reader.name() == QLatin1String("sheetPr")
            && reader.tokenType() == QXmlStreamReader::EndElement)) {
        reader.readNextStartElement();
        if (reader.tokenType() == QXmlStreamReader::StartElement && reader.name() == QLatin1String("pageSetUpPr")) {
            QXmlStreamAttributes attrs = reader.attributes();
            VFitTopage=attrs.value(QLatin1String("fitToPage")).toString().trimmed();
        }
    }
}
void WorksheetPrivate::loadXmlSheetViews(QXmlStreamReader &reader)
{
	...
	while (!reader.atEnd() && !(reader.name() == QLatin1String("sheetViews")
		...
		if (reader.tokenType() == QXmlStreamReader::StartElement && reader.name() == QLatin1String("sheetView")) {
            ...
            // Liu fei jin 2019/12/2
            VSheetView=attrs.value(QLatin1String("view")).toString().trimmed();
            VTopLeftCell=attrs.value(QLatin1String("topLeftCell")).toString().trimmed();
            VZoomScaleNormal=attrs.value(QLatin1String("zoomScaleNormal")).toString().trimmed();
            VWorkbookViewId=attrs.value(QLatin1String("workbookViewId")).toString().trimmed();
		}
	...
	while (!reader.atEnd()) {
		...
		if (reader.tokenType() == QXmlStreamReader::StartElement) {
			if (reader.name() == QLatin1String("dimension")) {
                ...
                QXmlStreamAttributes attributes = reader.attributes(); //20190619 by liufeijin
                d->MoodalignWithMargins=attributes.value("alignWithMargins").toString().trimmed();
                /*
                reader.readNextStartElement();
                ...
                */
                ...
                {
                    if ( (reader.tokenType() == QXmlStreamReader::StartElement) && (reader.name() == QLatin1String("oddHeader")) )
                        d->ModdHeader = reader.readElementText();

                    if ( (reader.tokenType() == QXmlStreamReader::StartElement) && (reader.name() == QLatin1String("oddFooter")) )
                        d->MoodFooter = reader.readElementText();

                }
               //  qDebug()<<d->ModdHeader<<d->MoodFooter;
